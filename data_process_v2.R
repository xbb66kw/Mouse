...
root <- "/Users/xbb/Desktop/mouse/"
file_directory <- "an194181_2013_01_18_data_struct/"
#Recorded trials
...
rq_series <- function(i_set, t_set) {
    ...
    for (i_ in i_set) { 
        for (t_ in t_set) {
            ...
            resi <- rq(y ~ x1 + x2, tau=0.90)$resi
            resi_second <- rq(y ~ x1 + x2, tau=0.60)$resi
            ...
            #if (sum(which(resi[bool_axis] >= 0)) >= 1) {
...
results2 <- rep(0, length(recorded_values[,1]))
...
for (i in 1:length(recorded_values[,1])) {
    ...
    results2[i] <- mid_$matching_rate_noNA
...
aa_ <- 60
#whole_matrix[1:a_,]
dd_one <- function(x) { sum(abs(x - y_fitting[1:aa_]), na.rm=TRUE) / sum(!is.na(x), na.rm=TRUE)}
dd_two <- function(x) { sum(abs(x - y_fitting[(aa_+1):a_]), na.rm=TRUE) / sum(!is.na(x), na.rm=TRUE)}
one <- apply(FUN=dd_one, whole_matrix[1:aa_,], MARGIN=2)
two <- apply(FUN=dd_two, whole_matrix[(aa_+1):a_,], MARGIN=2)

good_neurons_larger <- intersect(which(one %in% sort(one)[1:100]), which(two %in% sort(two)[1:100]))
#intersect(which(results %in% sort(results, de=TRUE)[1:100])[1:100], which(results_soft %in% sort(results_soft, de=TRUE)[1:100])[1:100])
...
for (frac_ in 1:e_) {
    ...
    para_negative_hard[s_] <- para_mid[which(results_mid == min(results_mid))[sum(results_mid == min(results_mid))]]
...
for (frac_ in 1:e_) {
    ...
    para_negative_soft[s_] <- para_mid[which(results_mid == min(results_mid))[sum(results_mid == min(results_mid))]]
...
if (which_to_use[1] <= which_to_use[2]) {
    ...
    tunning_set <- whole_matrix[(a_+1):b_,]    
    win_ <- which(frac_results_hard == min(frac_results_hard))[sum(frac_results_hard == min(frac_results_hard))]
    
    para_negative <- para_negative_hard[which(frac_results_hard == min(frac_results_hard))[sum(frac_results_hard == min(frac_results_hard))]]
} else {
    
    prediction_set_logistic <- stuff[[2]]#soft shape
    fitting_set <- soft_matrix[1:a_,]
    tunning_set <- soft_matrix[(a_+1):b_,]
    win_ <- which(frac_results_soft == min(frac_results_soft))[sum(frac_results_soft == min(frac_results_soft))]
    
    para_negative <- para_negative_soft[which(frac_results_soft == min(frac_results_soft))[sum(frac_results_soft == min(frac_results_soft))]]
