...
source("/Users/xbb/Desktop/mouse/linear_oga.r")
library(R.matlab)
library(quantreg)
...
false_prediction_neurons <- good_neurons#good_neurons_larger[quality_negative <= mean(quality_negative) - 0.5 * sd(quality_negative)]
...
base_prediction[rowSums(false_predictor) <= length(false_prediction_neurons) * 0.90] <- 0#######
sum(base_prediction - y_predict == 1 | base_prediction - y_predict == -1) / sum(base_prediction != 4 & base_prediction != 3)
